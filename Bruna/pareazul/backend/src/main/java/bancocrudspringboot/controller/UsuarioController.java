...
public class UsuarioController {
	...
	/* Em contrução para validar EMAIL
	 * // Inserir
	@PostMapping("/usuario")
	@ResponseStatus(HttpStatus.CREATED)
	public 	ResponseEntity	createCadastro(@RequestBody Usuario cadastro) {

		Optional <Usuario> cadastrobanco = usuarioRepository.findUsuarioByEmail(cadastro.getEmail());
		...
		if(validaEmail){
			return ResponseEntity.ok().body(cadastrobanco);
		}
		this.usuarioRepository.save(cadastro);



		return ResponseEntity.ok().body("Usuário Gravado!");
